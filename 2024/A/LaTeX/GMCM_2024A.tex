...
\begin{abstract}
	...
	\par 针对问题三，构建以主轴和塔架损伤均值最小化为双目标、带功率平衡与偏差约束的优化模型，每秒求解一次，并对比优化前后疲劳损伤总和显著降低。
	\par 针对问题四引入噪声和延迟的鲁棒补偿机制，采用状态估计与预测校正策略。本文方法兼顾实时性与精度，具有较强的工程实用性。

\keywords{疲劳损伤；滑动窗口；雨流计数法；NSGA-II 算法；鲁棒优化}
\end{abstract}
%\pagestyle{plain}
\tableofcontents
\newpage
...
目标函数
\begin{equation}
	\min J = \alpha \frac{1}{100}\sum_{i=1}^{100} D_s^i(t) + (1-\alpha)\frac{1}{100}\sum_{i=1}^{100} D_t^i(t)
\end{equation}
\par 约束条件
\begin{align}
