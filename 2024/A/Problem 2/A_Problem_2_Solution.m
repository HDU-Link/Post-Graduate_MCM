...
R = 63;                                                                     % 风轮半径 m
Jm = 3.8e7;                                                                 % 风轮转子转动惯量 kg·m^2
%% 3. 收集所有风机的数据
time_data = data_TS_WF.WF_2.WT{1}.time;
V = []; Tshaft = []; Ft = []; pitch = [];
omega_r = []; omega_dot = []; lambda = []; Cp = [];Ct = [];
for j = 1:2
    for i = 1:100
        ...
        V = [V; V_ji]; Tshaft = [Tshaft; Tshaft_ji]; Ft = [Ft; Ft_ji]; pitch = [pitch; pitch_ji];
        omega_r = [omega_r; omega_r_ji]; omega_dot = [omega_dot; omega_dot_ji];
        lambda_ji = omega_r_ji .* R ./ V_ji;                                % 叶尖速比
        lambda_star_ji = 1./(1./(lambda_ji + 0.08*pitch_ji) - 0.035./(pitch_ji.^3 + 1));
        Cp_ji = 2*(Tshaft_ji+Jm*omega_dot_ji)./ (pi*rho*R^2*V_ji.^3);
...
%% 4. 非线性拟合Cp = c1*(c2*(1/(λ+c3*β)-c4/(β**3+1))-c5*β-c6)*exp(-c7*(1/(λ+c3*β)-c4/(β**3+1))+c8*λ
cp_model = @(c, x) c(1)*(c(2)*(1./(x(:,1)+c(3)*x(:,2))-c(4)./(x(:,2).^3+1))-c(5)*x(:,2)-c(6)) ...
    .* exp(-c(7)*(1./(x(:,1)+c(3)*x(:,2))-c(4)./(x(:,2).^3+1)))+c(8)*x(:,1);
c0 = [0.5, 116, 0.08, 0.035, 0.4, 5, 21, 0.0068];                           % 初始猜测值
opts = optimoptions('lsqcurvefit', 'Display', 'iter', 'MaxFunctionEvaluations', 10000);
c_fitted = lsqcurvefit(cp_model, c0, [lambda(:), pitch(:)], Cp(:), [], [], opts);
fprintf('\nCp非线性模型拟合系数:\n');
fprintf('c1 = %.6f\n', c_fitted(1));fprintf('c2 = %.6f\n', c_fitted(2));
fprintf('c3 = %.6f\n', c_fitted(3));fprintf('c4 = %.6f\n', c_fitted(4));
fprintf('c5 = %.6f\n', c_fitted(5));fprintf('c6 = %.6f\n', c_fitted(6));
fprintf('c7 = %.6f\n', c_fitted(7));fprintf('c8 = %.6f\n', c_fitted(8));
Cp = cp_model(c_fitted, [lambda, pitch]);
...
%% 6. 计算预测值及误差
Tshaft_pred = (0.5*pi*rho*R^2*Cp.*V.^3)./omega_r - omega_dot*Jm;            % 主轴扭矩预测
...
%% 9. 输出最终公式
fprintf('【主轴扭矩公式】');fprintf('T_{shaft} = (0.5πρR²C_pV³)/ω_r - J_m·dω_r/dt，其中:\n');
fprintf(' C_p = 0.5176(116/λ_i - 0.4β - 5)exp(-21/λ_i) + 0.0068λ\n');
fprintf(' 1/λ_i = 1/(λ + 0.08β) - 0.035/(β³ + 1),λ = ω_r·R/V\n');
fprintf('【塔架推力公式】F_t = 0.5πρR²·C_t(λ,β)·V²，其中:\n');
fprintf(' C_t(λ,β) = %.6f + %.6f·λ + %.6f·β + %.6f·λ² + %.6f·λ·β + %.6f·β² + %.6f·λ³ + %.6f·β³\n', theta_poly);