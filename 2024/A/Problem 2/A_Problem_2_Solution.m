...
c0 = [0.5, 116, 0.08, 0.035, 0.4, 5, 21, 0.0068, 3e6];                      % 初始猜测值
opts = optimoptions('lsqcurvefit', 'Display', 'iter', 'MaxFunctionEvaluations', 1000);
C_fitted = lsqcurvefit(T_model, c0, ...
    [lambda(:), pitch(:), V(:), omega_r(:), omega_dot(:)], Tshaft(:), [], [], opts);
fprintf('\nCp非线性模型拟合系数:\n');fprintf('c1 = %.6f\n', C_fitted(1));
fprintf('c2 = %.6f\n', C_fitted(2));fprintf('c3 = %.6f\n', C_fitted(3));
...
fprintf(' 1/λ* = 1/(λ + %.6fβ) - %.6f/(β³ + 1),λ = ω_r·R/V, J_m=%.6f\n', [C_fitted(3:4),C_fitted(9)]);
