...
# 1. 读取数据
train_3ap_files = glob.glob("training_set_3ap_*.csv")
train_3ap_list = []
for f in train_3ap_files:
    df = pd.read_csv(f)
    train_3ap_list.append(df)
train_df = pd.concat(train_3ap_list, ignore_index=True)
test_path = "test_set_1_3ap.csv"
...
sns.heatmap(corr, cmap='coolwarm', vmin=-1, vmax=1, center=0,
            ...
            cbar_kws={'shrink': 0.7, 'label': 'Pearson Correlation (3AP)'})
...
ax.set_title('Feature Importance (3AP)')
plt.tight_layout()
plt.show()
